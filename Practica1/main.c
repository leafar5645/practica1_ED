...

	{
	  ...
	  if (expresion[l] == '+' || expresion[l] == '-' || expresion[l] == '*' || expresion[l] == '/' || expresion[l] == '^')	//si es un operador veremos las siguientes condiciones
...

		{
		  ...
		  while (Top (&mi_pila).car == '*' || Top (&mi_pila).car == '/' || Top (&mi_pila).car == '^' || Top (&mi_pila).car == '+' || Top (&mi_pila).car == '-')	//si el operador es + o - menos y en la pila hay operadores de mayor o igual importancia los va a sacr todos
